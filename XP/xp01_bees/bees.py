...
def honey_next_pos(position: int, h_pattern: str, hex_size: int, h_steps: list, i: int):
    ...
    if h_pattern == 'standing':
        ...
        step_increment = h_steps[2] - h_steps[1]
        pos = (position + step + step_increment * i) % hex_size
        if pos == 0:
            pos = hex_size
        return pos
...
def pollen_next_pos(position: int, p_pattern: str, hex_size: int, p_steps: list, i: int) -> int:
    ...
    if p_pattern == 'geometric':
        step = p_steps[1] - p_steps[0]
        multiplier = int((p_steps[1] / p_steps[0])) ** i
...
if __name__ == '__main__':
    ...
    # assert do_bees_meet(140, "1,2,4,8,16", "2,6,18,54,162") is True
    ...
    # print(do_bees_meet(140, "1,2,4,8,16", "2,6,18,54,162"))
